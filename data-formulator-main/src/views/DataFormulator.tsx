...
export const DataFormulatorFC = ({ }) => {
    ...
    let borderBoxStyle = {
        border: cilBase ? 'none' : `1px solid ${borderColor.view}`,
        borderRadius: cilBase ? 0 : radius.pill,
    ...
    const fixedSplitPane = ( 
        <Box sx={{display: 'flex', flexDirection: 'row', height: '100%'}}>
            ...
            <Box ref={containerRef} className="outer-allotment" sx={{
                    margin: cilBase ? 0 : '4px 8px 8px 8px', backgroundColor: 'white',
                    display: 'flex', height: cilBase ? '100%' : 'calc(100% - 12px)', flex: 1, minWidth: 0, flexDirection: 'column',
                    overflow: 'hidden',
                    position: 'relative',
    ...
    let footer = cilBase ? null : <Box sx={{ color: 'text.secondary', display: 'flex', 
    ...
    return (
        <Box sx={{ display: 'block', width: "100%", height: '100%', position: 'relative' }}>
            ...
            <DndProvider backend={HTML5Backend}>
                ...
                {selectedModelId == undefined && !cilBase && (
                    <Box sx={{
                        position: 'absolute',
                        top: 0,
                        left: 0,
