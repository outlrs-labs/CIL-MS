...
import { cilBase } from '../app/cilIntegration';
...
export const ReportView: FC = () => {
    ...
    const [savingToCil, setSavingToCil] = useState(false);
    const saveReportToCil = async () => {
        if (savingToCil) return;
        setSavingToCil(true);
        try {
            const canvas = await renderReportToCanvas();
            if (!canvas) throw new Error('Report is not ready to render.');
            const report = allGeneratedReports.find(item => item.id === currentReportId);
            if (!report) throw new Error('Select a report first.');
            const response = await fetch(cilBase + 'cil/save-report', {
                method: 'POST',
                ...
                body: JSON.stringify({
                    ...
                    charts: charts.filter(chart => report.selectedChartIds.includes(chart.id)),
                    model: activeModel?.id,
                    send_for_audit: true,
                    chart_images: Object.fromEntries(
                        report.selectedChartIds
                            .map(id => [id, getCachedChart(id)?.fullPngDataUrl || getCachedChart(id)?.thumbnailDataUrl])
                            .filter(([, png]) => Boolean(png)),
                    ),
                }),
            });
            const result = await response.json();
            if (!response.ok) throw new Error(result.detail || 'Could not save report.');
            showMessage(result.audit_status ? 'Report saved and sent to subsidiary audit.' : 'Report saved to the reporting workflow.');
            window.parent.postMessage({ type: 'cil-report-saved', id: result.id }, window.location.origin);
        } catch (error) {
            showMessage(error instanceof Error ? error.message : 'Report save failed.', 'error');
        } finally {
            setSavingToCil(false);
        }
    };
        ...
        <Box sx={{ height: '100%', width: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <Box sx={{ height: '100%', position: 'relative', overflow: 'hidden' }}>
                    ...
                    <Box sx={{
                        ...
                        {cilBase && !isGenerating && currentReportId && (
                            <Tooltip title="Save report and send for audit" placement="right">
                                <span>
                                    <IconButton
                                        aria-label="Save report and send for audit"
                                        disabled={savingToCil}
                                        onClick={saveReportToCil}
                                        sx={floatingPillSx}
                                    >
                                        <SaveIcon sx={{ fontSize: iconVar.lg }} />
                                    </IconButton>
                                </span>
                            </Tooltip>
                        )}
