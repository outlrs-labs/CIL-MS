...
import { FieldItem, Channel, EncodingItem, AggrOp, Chart, DictTable, EncodingDropResult } from "../components/ComponentType";
...
export function encodingFieldsForTable(fields: FieldItem[], table?: Pick<DictTable, 'id' | 'names'>): FieldItem[] {
    return fields.filter(field =>
        field.name !== "" && (!table || (field.tableRef === table.id && table.names.includes(field.name)))
    );
}
...
export const EncodingBox: FC<EncodingBoxProps> = function EncodingBox({ channel, chartId, tableId }) {
    ...
    let handleSelectOption = (option: FieldItem) => {
        ...
        const isAvailable = conceptShelfItems.some(field => field.id === option.id)
            && (!activeTable || (option.tableRef === activeTable.id && activeTable.names.includes(option.name)));
        if (isAvailable) {
            updateEncProp("fieldID", option.id);
        }
    }
    ...
    let availableFields = encodingFieldsForTable(conceptShelfItems, activeTable);
    ...
    let createConceptInputBox = <Autocomplete
        ...
        renderOption={(props, option) => {
            ...
            const dtype = activeTable?.metadata[option.name]?.type || Type.Auto;
            return (
                <Box
                    key={key}
                    {...liProps}
            ...
            <TextField {...params} variant="standard" autoComplete='off' placeholder={t('encoding.fieldPlaceholder')}
                onKeyDownCapture={(event) => {
                    ...
                    if (event.key === 'Enter') {
                        ...
                        if (!value || !availableFields.some(field => field.name === value)) {
    ...
    const filter = createFilterOptions<FieldItem>({ stringify: option => option.name });
