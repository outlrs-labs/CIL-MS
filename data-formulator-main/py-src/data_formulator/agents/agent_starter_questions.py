...
class StarterQuestionsAgent(object):
    ...
    def run(self, tables, primary_table=None, n=2):
        ...
        for choice in response.choices:
            ...
            # Some OpenAI-compatible providers legitimately return a choice
            # whose content is null (for example when no starter suggestions
            # are produced).  Logging must not turn that optional result into
            # a failed model request shown to the user.
            logger.debug("%s\n", content)
