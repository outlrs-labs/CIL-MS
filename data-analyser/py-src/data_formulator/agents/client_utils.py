...
def _normalize_tool_history(messages):
    """Return a copy with OpenAI-compatible tool history.

    Tool calls that complete without a display value are still successful. A
    few compatible endpoints require ``tool.content`` to contain at least one
    character. They also require every historical function ``arguments`` value
    to be a string containing valid JSON. Streaming providers occasionally
    leave that value as a dict or a truncated string; repair it before the next
    model turn so one malformed saved call cannot permanently break chat.
    """
    normalized = []
    for message in messages or []:
        ...
        if message.get("role") == "tool":
            ...
            if content is None or (isinstance(content, str) and not content.strip()) or content == []:
                updated = {**updated, "content": "No output."}
        ...
        if isinstance(tool_calls, list):
            ...
            for tool_call in tool_calls:
                ...
                if isinstance(arguments, str):
                    try:
                        ...
                        try:
                            ...
                            parsed = {}
                        repaired = json.dumps(parsed if isinstance(parsed, dict) else {})
                elif isinstance(arguments, dict):
                    repaired = json.dumps(arguments)
                else:
                    repaired = "{}"
                ...
                if repaired != arguments:
                    ...
                    repaired_calls.append(tool_call)
            if changed:
                updated = {**updated, "tool_calls": repaired_calls}

        normalized.append(updated)
    return normalized


# Backwards-compatible name retained for existing integrations and tests.
_normalize_empty_tool_messages = _normalize_tool_history
...
class Client(object):
    ...
    def __init__(self, endpoint, model, api_key=None, api_base=None, api_version=None,
                 timeout_seconds=30, fallback_configs=None):
        
        self.endpoint = endpoint
        self.model = model
        self.params = {}
        self.params["timeout"] = max(5, min(int(timeout_seconds), 120))
        self.fallback_clients = [
            Client(
                cfg["endpoint"], cfg["model"], cfg.get("api_key"),
                cfg.get("api_base"), cfg.get("api_version"),
                timeout_seconds=cfg.get("timeout_seconds", 45),
            )
    ...
    def from_config(cls, model_config: dict[str, str]):
        ...
        return cls(
            ...
            fallback_configs=model_config.get("fallbacks") or [],
        )

    def _provider_attempts(self):
        yield self
        yield from self.fallback_clients

    @staticmethod
    def _first_chunk_or_raise(response):
        iterator = iter(response)
        first = next(iterator)
        return itertools.chain((first,), iterator)

    def _run_with_fallback(self, operation, stream):
        """Use a fallback only if the primary fails before streaming output."""
        last_error = None
        for client in self._provider_attempts():
            try:
                response = operation(client)
                return self._first_chunk_or_raise(response) if stream else response
    ...
    def _dispatch(self, *, messages, stream, params, tools=None, extra=None):
        ...
        # Some OpenAI-compatible providers reject an empty tool result even
        # though other providers accept it. Data operations can legitimately
        # produce no textual output, so normalize only those empty tool
        # messages to a small, truthful sentinel before either the primary or
        # fallback request is dispatched.
        messages = _normalize_tool_history(messages)
    ...
    def get_completion(self, messages, stream=False, reasoning_effort="low",
        ...
        def attempt(client):
            ...
            try:
                return client._dispatch(messages=messages, stream=stream, params=params)
            except Exception as e:
                err = str(e)
                if client._is_reasoning_effort_error(err):
                    params.pop("reasoning_effort", None)
                    return client._dispatch(messages=messages, stream=stream, params=params)
                if client._is_image_deserialize_error(err, client._messages_contain_images(messages)):
                    return client._dispatch(messages=client._strip_images_from_messages(messages), stream=stream, params=params)
                raise
        return self._run_with_fallback(attempt, stream)
    ...
    def get_completion_with_tools(self, messages, tools, stream=False,
        ...
        def attempt(client):
            ...
            params["reasoning_effort"] = reasoning_effort
            try:
                return client._dispatch(messages=messages, stream=stream,
                ...
                if client._is_reasoning_effort_error(err):
                    ...
                    return client._dispatch(messages=messages, stream=stream,
                                            params=params, tools=tools, extra=kwargs)
                if client._is_image_deserialize_error(err, client._messages_contain_images(messages)):
                    return client._dispatch(messages=client._strip_images_from_messages(messages), stream=stream,
                                            params=params, tools=tools, extra=kwargs)
                raise
        return self._run_with_fallback(attempt, stream)
