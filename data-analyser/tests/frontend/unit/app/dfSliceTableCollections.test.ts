...
describe("split table collections", () => {
  ...
  it("recovers selectable fields from input snapshots when a saved shelf is empty", () => {
    const state = dataFormulatorReducer(
      undefined,
      dfActions.loadState({
        inputTables: [{
          ...
          id: "ocr_rows",
          displayId: "OCR rows",
          source: { kind: "workspace", tableId: "ocr_rows" },
          snapshot: {
            columns: [
    ...
    expect(state.conceptShelfItems).toEqual([
      { id: "original--ocr_rows--page", name: "page", source: "original", tableRef: "ocr_rows" },
      { id: "original--ocr_rows--record_type", name: "record_type", source: "original", tableRef: "ocr_rows" },
    ]);
  });
