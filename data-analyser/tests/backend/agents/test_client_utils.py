...
from data_formulator.agents.client_utils import Client, _normalize_empty_tool_messages


def test_normalize_empty_tool_messages_preserves_history_and_fills_tool_results():
    messages = [
        {"role": "user", "content": "chart this"},
        {"role": "tool", "tool_call_id": "one", "content": ""},
        {"role": "tool", "tool_call_id": "two", "content": None},
        {"role": "tool", "tool_call_id": "three", "content": "done"},
    ]

    normalized = _normalize_empty_tool_messages(messages)

    assert normalized[0] is messages[0]
    assert normalized[1]["content"] == "No output."
    assert normalized[2]["content"] == "No output."
    assert normalized[3] is messages[3]
    assert messages[1]["content"] == ""


def test_normalize_tool_history_repairs_invalid_function_arguments():
    messages = [
        {
            "role": "assistant",
            "content": None,
            "tool_calls": [{
                ...
                "function": {
                    "name": "visualize",
                    "arguments": "{'chart_type': 'bar', 'x': 'record_type'}",
                },
            }],
        },
    ...
    assert arguments == '{"chart_type": "bar", "x": "record_type"}'
    assert messages[0]["tool_calls"][0]["function"]["arguments"].startswith("{'chart_type'")


def test_normalize_tool_history_replaces_truncated_arguments_with_empty_object():
    messages = [{
        "role": "assistant",
        "tool_calls": [{
            "id": "call-1",
            "type": "function",
            "function": {"name": "visualize", "arguments": '{"chart_type":'},
        }],
    }]
