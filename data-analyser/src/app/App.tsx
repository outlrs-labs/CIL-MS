...
import { cilAnalysisId, cilBase } from './cilIntegration';
...
export const toolName = "CIL Analytics"
    ...
    return (
        <>
            <Tooltip title={t('workspace.sessionTooltip', { name: activeWorkspace?.id || '' })} placement="bottom">
                <Box 
                    onClick={() => !cilBase && !diskPersistenceDisabled && setPickerOpen(true)}
                    sx={{ 
    ...
    return (
        <Box sx={{
            ...
            <Box sx={{
                ...
                {!cilBase && <AppBar position="static">
...
export const AppFC: FC<AppFCProps> = function AppFC(appProps) {
    ...
    // The embedded workbench is part of the CIL dashboard and follows its
    // green appearance accent. Standalone Data Formulator keeps the user's
    // selected palette.
    const activePaletteKey = cilBase
        ? 'cil'
        : ((rawPaletteKey && palettes[rawPaletteKey]) ? rawPaletteKey : defaultPaletteKey);
    ...
    useEffect(() => {
        apiRequest(getUrls().APP_CONFIG)
            .then(async ({ data }) => {
                ...
                if (cilBase) {
                    const result = await loadWorkspace(cilAnalysisId);
                    if (result) {
                        dispatch(dfActions.loadState({
                            ...result.state,
                            activeWorkspace: { id: cilAnalysisId, displayName: result.displayName },
                        }));
                    }
                }
    ...
    useEffect(() => {
        ...
        if (cilBase && serverConfig?.IDENTITY) {
            dispatch(dfActions.setIdentity(serverConfig.IDENTITY as any));
            setAuthChecked(true);
            return;
        }
