...
const getStoredSelectedModelId = (): string | undefined => {
    try {
        return globalThis.localStorage?.getItem('df_selected_model') || undefined;
    } catch {
        return undefined;
    }
};
...
export const dataFormulatorSlice = createSlice({
    ...
    reducers: {
        ...
        loadState: (state, action: PayloadAction<any>) => {
            ...
            // Input-table rows live in the workspace backend, so persisted
            // sessions contain compact column snapshots rather than DictTable
            // objects. Older/imported CIL sessions can consequently have an
            // empty concept shelf even though their snapshots contain every
            // column. Encoding dropdowns and grid-header drag/drop both resolve
            // fields through this shelf, so rebuild any missing original fields
            // while loading the workspace.
            const persistedConcepts = Array.isArray(saved.conceptShelfItems)
                ? saved.conceptShelfItems
                : [];
            const recoveredConcepts: FieldItem[] = [];
            for (const table of (saved.inputTables || [])) {
                for (const column of (table.snapshot?.columns || [])) {
                    if (!column?.name) continue;
                    recoveredConcepts.push({
                        id: `original--${table.id}--${column.name}`,
                        name: column.name,
                        source: 'original',
                        tableRef: table.id,
                    } as FieldItem);
            ...
            for (const table of (saved.derivedTables || [])) {
                recoveredConcepts.push(...getDataFieldItems(table));
            }
            const conceptsById = new Map<string, FieldItem>();
            // Persisted concepts may contain user-authored properties; let
            // them win while using recovered fields only as a backfill.
            for (const field of [...recoveredConcepts, ...persistedConcepts]) {
                if (field?.id) conceptsById.set(field.id, field);
            }
            ...
            return {
                ...
                conceptShelfItems: [...conceptsById.values()],
