...
export const SimpleChartRecBox: FC<{ onInputFocus?: () => void }> = function ({ onInputFocus }) {
    ...
    // Chat keeps Data Formulator's familiar blue identity inside the embedded
    // CIL workbench; the canvas, fields, charts and selection UI use CIL green.
    const chatAccent = cilBase ? '#0078d4' : theme.palette.primary.main;
    const chatAccentDark = cilBase ? '#0067b8' : theme.palette.primary.dark;
    const { t } = useTranslation();
    ...
    const inputBox = (
        <Card className="cil-chat-composer" ref={inputCardRef} variant="outlined" sx={{
            ...
            borderColor: chatInputFocused ? chatAccent : borderColor.divider,
            backgroundColor: theme.palette.background.paper,
            ...
            boxShadow: chatInputFocused
                ? `0 0 0 2px ${alpha(chatAccent, 0.15)}, 0 2px 10px rgba(32, 33, 36, 0.14)`
        ...
        >
            ...
            <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 0.5 }}>
                    ...
                    <Tooltip title={t('chartRec.attachContext', { defaultValue: 'Attach context (image or file)' })}>
                        <IconButton
                            ...
                            sx={{
                                ...
                                color: theme.palette.text.secondary,
                                borderRadius: '4px',
                                '&:hover': { color: chatAccent, backgroundColor: alpha(chatAccent, 0.06) },
                            }}
                        >
                ...
                {isChatFormulating ? (
                    <CircularProgress size={18} sx={{ m: 0.5, color: chatAccent }} />
                ) : (
                    <>
                        {cilBase ? <>
                            <Button
                                size="small"
                                variant="text"
                                startIcon={<UploadFileOutlinedIcon sx={{ fontSize: iconVar.md }} />}
                                onClick={() => window.parent.postMessage({ type: 'cil-open-data' }, window.location.origin)}
                                sx={{ textTransform: 'none', whiteSpace: 'nowrap', minWidth: 0, px: 0.75, color: chatAccent }}
                            >
                                Upload / OCR
                            </Button>
                            <Button
                                size="small"
                                variant="text"
                                startIcon={<EditOutlinedIcon sx={{ fontSize: iconVar.md }} />}
                                disabled={!focusedTableId || isChatFormulating || !!pendingClarification}
                                onClick={() => window.parent.postMessage({ type: 'cil-generate-report' }, window.location.origin)}
                                sx={{ textTransform: 'none', whiteSpace: 'nowrap', minWidth: 0, px: 0.75, color: chatAccent }}
                            >
                                Make the report
                            </Button>
                            ...
                            <span>
                                <IconButton
                                    ...
                                    sx={{
                                        p: 0,
                                        width: 28, height: 28,
                                        bgcolor: canSend ? chatAccent : 'transparent',
                                        color: canSend ? 'common.white' : chatAccent,
                                        '&:hover': {
                                            bgcolor: canSend ? chatAccentDark : 'transparent',
                                        },
                                        '&.Mui-disabled': {
                                            bgcolor: 'transparent',
                                            color: 'text.disabled',
